...
build = {
	...
	welcomeModal: function() {
		...
		modal += "<div class='message_overlay'>";
		modal +=	"<div class='welcome center'>";
		modal +=		"<h1>Bienvenue sur le site Bird Capture</h1>";
		modal +=		"<div class='visiteur'>Vous pouvez entrer en temps que visiteur :</div>";
		modal +=		"<div class='admin'>Ou vous connectez pour acc&#233;der &#224; l'administration :</div>";
		modal +=		"<a class='close button active'>Come in</a>";
		modal +=		"<div class='sign_in'>";
		modal +=			"<input id='username' type='text' name='' value='' placeholder='username'>";
		modal +=			"<input id='password' type='password' name='' value='' placeholder='password'>";
		modal +=		"</div>";
	...
	infoboxPhoto: function(photoJSON, forView) {
		...
		editColorHTML = (forView===true||lychee.publicMode) ? "" : " " + build.editIcon("edit_color");
		editBirdsizeHTML = (forView===true||lychee.publicMode) ? "" : " " + build.editIcon("edit_birdsize");
		...
		infos = [
			...
			["", "Image"],
			["Color", photoJSON.color + editColorHTML],
			["Birdsize", photoJSON.birdsize + editBirdsizeHTML],
			["Size", photoJSON.size],
