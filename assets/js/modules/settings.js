...
var settings = {
		...
		modal.show("Change Sorting",
			"Sort photos by \
				<select id='settings_type'> \
					...
					<option value='birdsize'>Bird Size</option> \
	...
	setFolderSort: function(folder) {

		params = "setFolderSorting&sort=" + folder;
		lychee.api(params, function(data) {

			if (data===true) {
				lychee.folder = folder;
				lychee.load();
			} else lychee.error(null, params, data);
	...
	takePicture: function(){
		...
		lychee.api("getRasp", function(data) {
			item = "Take a picture on the raspberry \
				"; 
			...
			modal.show("Take a picture",item, buttons);
		});
		
	},
	
	setTurnRaspberry: function(){
		var buttons,
			capture;

		buttons = [
			["Turn off/up Raspberry Pi", function() {
				var capture = new Array();
				capture[0] = $("select#address").val();
				capture[1] = $("select#etat").val();

				params = "turnOpenCV&address=" + capture[0] + "&etat=" + capture[1];
				lychee.api(params, function(data) {
					if (data===true) {
						lychee.load();
					} else {lychee.error(null, params, data)};
		...
		lychee.api("getRasp", function(data) {
			...
			modal.show("Stop or start Open CV",item, buttons);
