...
photo = {
	...
	setColor: function(photoID) {

		var oldColor = photo.json.color,
			color,
			params,
			buttons;
		...
		buttons = [
			["Set Color", function() {
				...
				color = $(".message input.text").val();

				if (color.length<20 && color.length>0) {

					if (visible.photo()) {
						photo.json.color = color;
						view.photo.color();
					}

					params = "setPhotoColor&photoID=" + photoID + "&color=" + escape(color);
					lychee.api(params, function(data) {
				...
				} else loadingBar.show("error", "New color to short or too long. Please try again!");

			}],
			["Cancel", function() {}]
		];
		modal.show("Set Color", "Please enter a color for this photo: <input class='text' type='text' placeholder='Color' value='" + oldColor + "'>", buttons);

	},
	
	setBirdsize: function(photoID) {

		var oldBirdsize = photo.json.birdsize,
			birdsize,
			params,
			buttons;

		buttons = [
			["Set Birdsize", function() {

				birdsize = $(".message input.text").val();

				if (birdsize=='petit' || birdsize=='moyen' || birdsize=='grand') {

					if (visible.photo()) {
						photo.json.birdsize = birdsize;
						view.photo.birdsize();
					}

					params = "setPhotoBirdsize&photoID=" + photoID + "&birdsize=" + escape(birdsize);
					lychee.api(params, function(data) {
				...
				} else loadingBar.show("error", "Please, enter 'petit', 'moyen' or 'grand'!");

			}],
			["Cancel", function() {}]
		];
		modal.show("Set Birdsize", "Please enter 'petit', 'moyen' or 'grand' for this photo: <input class='text' type='text' placeholder='Birdsize' value='" + oldBirdsize + "'>", buttons);
