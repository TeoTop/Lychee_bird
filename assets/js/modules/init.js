...
$(document).ready(function(){
	...
	$("#button_signin").on(event_name, contextMenu.signin);
	$("#button_settings").on(event_name, contextMenu.settings);
	$("#button_tri_nom").on(event_name, function() {settings.setFolderSort('name')});
	$("#button_tri_couleur").on(event_name, function() {settings.setFolderSort('color')});
	$("#button_tri_taille").on(event_name, function() {settings.setFolderSort('size')});
	...
	$("#infobox")
		...
		.on(event_name, "#edit_color", function() { photo.setColor(photo.getID()) })
		.on(event_name, "#edit_birdsize", function() { photo.setBirdsize(photo.getID()) });
	...
	Mousetrap.bindGlobal(['esc', 'command+up'], function(e) {
		...
		else if (visible.welcome()&&$(".welcome .close").length>0) modal.close();
	...
	$(document)
		...
		.on(event_name, ".welcome .close", modal.close)
