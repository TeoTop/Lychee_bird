...
function createTables($database) {
	...
	if (!$database->query("SELECT * FROM lychee_albums;")) {
		...
		$query = "
			...
			(1, 'petit', 'Dossier regroupant les oiseaux de petite taille', '$sysdate', 1, '', 'size'),
			(2, 'moyen', 'Dossier regroupant les oiseaux de taille moyenne', '$sysdate', 1, '', 'size'),
			(3, 'grand', 'Dossier regroupant les oiseaux de grande taille', '$sysdate', 1, '', 'size');
		";
	...
	if (!$database->query("SELECT * FROM lychee_rasp;")) {

		$query = "

			CREATE TABLE `lychee_rasp` (
