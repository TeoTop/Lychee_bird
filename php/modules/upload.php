...
function upload($files, $albumID) {
	...
	foreach ($files as $file) {
	    ...
	    // Read infos
	    $info = getInfo($photo_name, $birdname);
		...
		if($info['albums_color'] != '' &&	$info['albums_size'] != ''){
			$query = "INSERT INTO lychee_photos (id, title, url, description, color, birdsize, type, width, height, size, sysdate, systime, iso, aperture, make, model, shutter, focal, takedate, taketime, thumbUrl, album, public, star, import_name)
				VALUES (
					'" . $idsize . "',
					'" . $info['title'] . "',
					'" . $photo_name . "',
...
function getInfo($filename, $birdname) {
	...
	$return['albums_color'] = '';
	$return['albums_size'] = '';
	$full_name = explode('@', $birdname);
	if(count($full_name)==3){
		$return['title'] = htmlentities($full_name[0]);
		$return['color'] = htmlentities($full_name[1]);
		$return['birdsize'] = htmlentities($full_name[2]);
		
		$return['albums_color'] = idColorAlbum($return['color']);
		
		$return['albums_size'] = idBirdsizeAlbum($return['birdsize']);	
