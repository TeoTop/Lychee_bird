...
function getAlbums($public, $folder) {
    ...
    // Albums
    if ($public) $query = "SELECT * FROM lychee_albums WHERE public = 1 AND type='".$folder."' ORDER BY title;";
    else $query = "SELECT * FROM lychee_albums WHERE type='".$folder."' ORDER BY title;";
    ...
    while($row = $result->fetch_object()) {
        ...
        $return["content"][$row->id]['title'] = str_replace('_', ' ',$row->title);;
...
function deleteAlbum($albumID) {
	...
	$result1 = $database->query("SELECT type FROM lychee_albums WHERE id = '$albumID';");
    while($row1 = $result1->fetch_object()) {
		if ($albumID!=0 && ($row1->type)!='size') {
			$result2 = $database->query("DELETE FROM lychee_albums WHERE id = '$albumID';");
